...
la memoria. Este apéndice documenta el conjunto de \textit{scripts} que conforman
...
día de hoy, TMDB y DBLP, y se describe el \textit{script} de poblado, común a
ambos, que materializa los objetos en las cinco tablas del esquema.

\section{\textit{Pipeline} general}
...
permite procesarlo en \textit{streaming} sin cargarlo entero en memoria) y se
estructura en dos partes. La primera línea es un objeto de cabecera con
la lista de colecciones presentes en el \textit{dataset} (nombre e identificador
...
Código~\ref{lst:jsonlSchema} reproduce un fragmento ilustrativo del
formato.
...
estructura, lo que hace que la etapa de ingesta sea un único \textit{script}
...
La extracción se apoya en un par de \textit{scripts} en Python independientes,
uno por etapa. El primero descarga los identificadores diarios
publicados por TMDB y, a continuación, consulta el \textit{endpoint} de detalle
...
de versiones. La descarga utiliza un \textit{pool} de corrutinas asíncronas con
...
de la base de datos y de respuesta del \textit{frontend} prohibitivos para una
...
Con \texttt{TOP\_K = 500}, el \textit{dataset} resultante (denominado
...
usuario tiene más probabilidades de querer navegar. El mismo \textit{script}
...
desde este archivo mediante un tamaño de \textit{bucket} por campo.

Centralizar la configuración en este archivo permite extender el
\textit{dataset} con nuevos campos sin tocar el código Python, y facilita además
...
y permite construir un \textit{dataset} alineado con la comunidad universitaria
...
cuando está activo, el \textit{script} conserva únicamente personas cuya
...
tamaño del \textit{dataset} y produce un grafo autoconsistente alineado con el
...
La etapa final del \textit{pipeline} es común a ambas fuentes y se implementa en
\texttt{scripts/\allowbreak populate\_\allowbreak db\_\allowbreak jsonl.py}. El \textit{script} consume un archivo
...
nombre del \textit{dataset} y la ruta al JSONL pueden pasarse como argumentos
de la línea de comandos, lo que permite cargar varios \textit{datasets}
independientes sobre la misma base de datos.

\section{Ejecución completa desde cero}

Para reproducir el \textit{pipeline} completo desde un repositorio recién
clonado los pasos son los descritos en el Código~\ref{lst:pipelineRun}.
...
\begin{lstlisting}[style=bashStyle, caption=Ejecución del \textit{pipeline} completo, label=lst:pipelineRun]
