...
volcado público completo y produce el dataset reducido que se carga finalmente en la base de
...
En el back-end Java, mi parte ha sido la \textbf{capa de persistencia y los controladores
infraestructurales}. Concretamente, he sido responsable de \texttt{DatabaseService} y de la
lógica de conexión a PostgreSQL, del controlador de sesión \texttt{SessionServlet}, del
controlador central de transiciones \texttt{NavigationServlet} (que orquesta todas las acciones
del motor) y del \texttt{CORSFilter} que permite el consumo desde el cliente web en el escenario
de desarrollo. También he implementado los servlets de cabecera del dominio
...
En el frontend, mi aportación ha sido menor y se ha limitado a colaborar en la primera versión
de la capa de comunicación con la API (\texttt{src/api.js}) y en la integración con los servlets
durante la depuración de los problemas de CORS y de manejo de sesión. El resto del frontend
(componentes, diseño visual, evaluación con usuarios) ha sido responsabilidad de mi compañero.

Para el despliegue, he sido responsable de los \texttt{Dockerfile} del frontend y del backend,
...
\textbf{desarrollo completo del frontend}, el \textbf{diseño de la experiencia de usuario} y
...
El \textbf{frontend} ha sido íntegramente mi responsabilidad. He diseñado e implementado la
...
superior, estados de \textit{hover}/\textit{focus} y feedback visual de las interacciones) es
...
En el back-end Java, durante la fase del prototipo, mi parte ha sido la lógica de
...
del frontend} y \textbf{Operaciones de conjuntos} dentro del capítulo de Desarrollo, y he
