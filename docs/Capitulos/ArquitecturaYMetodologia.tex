...
El motor opera sobre un universo de datos estructurado en datasets, colecciones y entidades. Fijamos los
siguientes conjuntos:

\begin{itemize}
  \item $\mathcal{D}$: conjunto de \emph{datasets}. Cada dataset agrupa colecciones cargadas desde una misma fuente.
  \item $\mathcal{C}$: conjunto de \emph{colecciones}.
...
\begin{align*}
  \mathrm{ds} &: \mathcal{C} \to \mathcal{D} & &\text{(dataset al que pertenece una colección)}\\
...
La sesión de un usuario explora un único dataset; fijamos por tanto un dataset $d_{0} \in \mathcal{D}$ y
denotamos $\mathcal{C}_{d_{0}} = \{c \in \mathcal{C} \mid \mathrm{ds}(c) = d_{0}\}$.

\subsubsection*{Estado de exploración}

Un \emph{estado} es una tupla
\[
  s \;=\; \langle\, c,\; F^{+},\; F^{-},\; R^{+},\; R^{-},\; L \,\rangle
\]
donde:
\begin{itemize}
  \item $c \in \mathcal{C}_{d_{0}}$ es la \emph{colección activa}.
  \item $F^{+}, F^{-} : \mathcal{C}_{d_{0}} \times \mathcal{K} \to \mathcal{P}(\mathcal{V})$ son los
    \emph{filtros de metadatos}, positivos y negativos, indexados por colección y clave.
  \item $R^{+}, R^{-} : \mathcal{C}_{d_{0}} \times \mathcal{C}_{d_{0}} \times \mathcal{R} \to \mathcal{P}(\mathcal{E})$
    son los \emph{filtros de referencia}, positivos y negativos, indexados por colección activa, colección
    referenciada y razón.
  \item $L = [\ell_{1}, \ldots, \ell_{n}]$ es una secuencia ordenada de \emph{links} (definidos a continuación).
\end{itemize}

Denotamos por $\mathcal{S}$ el conjunto de estados posibles. Esta tupla se corresponde literalmente con la clase
\texttt{State} del backend: el campo $c$ es \texttt{currentCollectionId}; los mapas $F^{+}, F^{-}, R^{+}, R^{-}$
son \texttt{mfilters}, \texttt{notMfilters}, \texttt{rfilters} y \texttt{notRfilters}; y $L$ es la lista
\texttt{links}. El dataset $d_{0}$ se almacena además en el campo \texttt{datasetId}.

\subsubsection*{Link}

Un \emph{link} representa una transición entre colecciones registrada como parte del historial de navegación
del estado. Es una tupla
\[
...
\begin{itemize}
  ...
  \item $c_{\text{from}} \in \mathcal{C}_{d_{0}}$ es la \emph{colección origen} de la transición, es decir, la
...
Esta tupla corresponde con la clase \texttt{Link}: el campo $d$ se materializa en el booleano \texttt{forward},
y $s'$ es la copia profunda del estado capturada en el momento de crear el enlace.

\subsubsection*{Contexto de exploración}

El contexto de una sesión es una tupla
\[
...
\begin{itemize}
  ...
  \item $T$ es una pila auxiliar de links \emph{forward} ya aplicados (\texttt{linkStack}), que permite a
    \texttt{goback} recuperar la colección origen del último enlace.
\end{itemize}

\subsubsection*{Operaciones primitivas}

Para cada operación se indica su signatura, su precondición y su postcondición. Las operaciones marcadas con
$\rightsquigarrow$ actúan sobre el estado activo $s_{\text{cur}}$ del contexto; las marcadas con $\Rightarrow$
modifican el contexto completo. Por brevedad, cuando se modifica una función como $F^{+}$, escribimos
$F^{+}[(c,k) \mapsto V]$ para denotar la función igual a $F^{+}$ salvo en $(c,k)$, donde toma el valor $V$.
...
\begin{enumerate}
  \item $\textsc{addMfilter}(s, k, v) \rightsquigarrow s'$
    \begin{itemize}
      ...
      \item Postcondición: $s'$ es idéntico a $s$ salvo que
      $F^{+}{}' \;=\; F^{+}[\,(c,k) \mapsto F^{+}(c,k) \cup \{v\}\,]$,
      donde $c$ es la colección activa de $s$.
    \end{itemize}

  \item $\textsc{addNotMfilter}(s, k, v) \rightsquigarrow s'$
    \begin{itemize}
      \item Signatura: $\mathcal{S} \times \mathcal{K} \times \mathcal{V} \to \mathcal{S}$.
      \item Postcondición: análoga a la anterior actualizando $F^{-}$ en lugar de $F^{+}$.
    \end{itemize}

  \item $\textsc{addRfilter}(s, c', r, e) \rightsquigarrow s'$
    \begin{itemize}
      \item Signatura: $\mathcal{S} \times \mathcal{C}_{d_{0}} \times \mathcal{R} \times \mathcal{E} \to \mathcal{S}$.
      \item Precondición: $\mathrm{coll}(e) = c'$.
      \item Postcondición: $R^{+}{}'(c, c', r) = R^{+}(c, c', r) \cup \{e\}$; el resto de $R^{+}$ no cambia. La
        operación $\textsc{addNotRfilter}$ es análoga actualizando $R^{-}$.
    \end{itemize}

  \item $\textsc{link}(s, c', r) \rightsquigarrow s'$
    \begin{itemize}
      \item Signatura: $\mathcal{S} \times \mathcal{C}_{d_{0}} \times \mathcal{R} \to \mathcal{S}$.
      \item Precondición: existe al menos una entidad $e \in \llbracket s \rrbracket$ y una entidad
        $e' \in \mathcal{E}$ con $\mathrm{coll}(e') = c'$ tales que $(e, e', r) \in \mathrm{Ref}$.
      \item Postcondición: $s'$ tiene la misma información de filtros que $s$ y además
        \[
          c' \text{ pasa a ser la colección activa,} \qquad
          L' \;=\; L \,\mathbin{::}\, \langle \mathsf{fwd},\, c,\, r,\, s \rangle.
        \]
        El contexto registra adicionalmente el nuevo link en la pila $T$.
    \end{itemize}

  \item $\textsc{goback}(\Sigma) \Rightarrow \Sigma'$
    \begin{itemize}
      \item Signatura: contexto $\to$ contexto.
      \item Precondición: $T$ no está vacía; sea $\ell = \langle \mathsf{fwd}, c_{\text{prev}}, r_{\text{prev}}, s_{\text{snap}}\rangle$
        el elemento en la cima de $T$.
      \item Postcondición: $T' = \mathrm{pop}(T)$; el estado activo $s_{\text{cur}}'$ se obtiene de $s_{\text{cur}}$
        cambiando la colección activa a $c_{\text{prev}}$ y añadiendo a $L$ un link
        $\langle \mathsf{bwd}, c, r_{\text{prev}}, s_{\text{cur}} \rangle$, donde $c$ es la colección activa de $s_{\text{cur}}$.
    \end{itemize}
  ...
  \item $\textsc{save}(\Sigma) \Rightarrow \Sigma'$ y $\textsc{restore}(\Sigma) \Rightarrow \Sigma'$
    \begin{itemize}
      ...
      \item $\textsc{restore}$ desapila el último estado guardado y lo restaura como $s_{\text{cur}}$, siempre que
        $|H| > 1$.
    \end{itemize}

  \item $\textsc{union}(\Sigma, c') \Rightarrow \Sigma'$
    \begin{itemize}
      \item Signatura: contexto $\times\, \mathcal{C}_{d_{0}} \to$ contexto.
      \item Postcondición: $U' = U \mathbin{::} s_{\text{cur}}$; $H' = \emptyset$; $s_{\text{cur}}'$ se reinicializa
        como estado limpio con colección activa $c'$ (es decir, sin filtros y con $L = []$).
    \end{itemize}
\end{enumerate}

\subsubsection*{Semántica: resultados de un estado}

Lo más importante del modelo es decidir \emph{qué conjunto de entidades representa un estado}. Definimos una
función semántica
\[
  \llbracket \cdot \rrbracket \;:\; \mathcal{S} \longrightarrow \mathcal{P}(\mathcal{E})
\]
de manera recursiva sobre la lista de links. Empezamos por los predicados de filtrado, que actúan sobre una
entidad $e$ con colección $c = \mathrm{coll}(e)$:

\begin{align*}
  \Phi^{+}(s, e) &\;\equiv\; \forall (k, V) \in F^{+}(c, \cdot) \;\; \exists v \in V :\; (k, v) \in \mathrm{meta}(e),\\
  \Phi^{-}(s, e) &\;\equiv\; \forall (k, V) \in F^{-}(c, \cdot) \;\; \forall v \in V :\; (k, v) \notin \mathrm{meta}(e),\\
  \Psi^{+}(s, e) &\;\equiv\; \forall (c', r, E) \in R^{+}(c, \cdot, \cdot) \;\; \forall e' \in E :\; (e, e', r) \in \mathrm{Ref},\\
  \Psi^{-}(s, e) &\;\equiv\; \forall (c', r, E) \in R^{-}(c, \cdot, \cdot) \;\; \forall e' \in E :\; (e, e', r) \notin \mathrm{Ref}.
\end{align*}

(La notación $F^{+}(c, \cdot)$ designa el conjunto de pares $(k, V)$ con $V = F^{+}(c, k) \neq \emptyset$; análogamente
$R^{+}(c, \cdot, \cdot)$ designa las tripletas $(c', r, E)$ con $E = R^{+}(c, c', r) \neq \emptyset$.)

\paragraph{Caso base.} Si $L = []$,
\[
  \llbracket s \rrbracket \;=\; \bigl\{\, e \in \mathcal{E} \;\bigm|\; \mathrm{coll}(e) = c \,\land\, \Phi^{+}(s,e) \,\land\, \Phi^{-}(s,e) \,\land\, \Psi^{+}(s,e) \,\land\, \Psi^{-}(s,e) \,\bigr\}.
\]

\paragraph{Caso recursivo.} Si $L = L' \mathbin{::} \ell$ con $\ell = \langle d, c_{\text{from}}, r, s' \rangle$,
sea $s_{L'}$ el estado obtenido de $s$ reemplazando $L$ por $L'$. Entonces
\[
  \llbracket s \rrbracket \;=\; \bigl\{\, e \in \llbracket s_{L'} \rrbracket \;\bigm|\; \exists e'' \in \llbracket s' \rrbracket \;:\; \Theta_{d}(e, e'', r) \,\bigr\},
\]
donde
\[
...
Esta definición recursiva es exactamente la que se materializa en la sobrecarga recursiva
\texttt{appendFilterClauses} de \texttt{NavigationDAO}: la rama no recursiva delega en
\texttt{appendStateFilters} ($\Phi^{\pm}$ y $\Psi^{\pm}$ se traducen en cláusulas
\texttt{EXISTS} y \texttt{NOT EXISTS} sobre las tablas \texttt{metadata} y \texttt{reference}); la rama recursiva
añade, por cada link $\ell$, una subconsulta sobre \texttt{reference} que liga las entidades del nivel actual con
las del estado anterior, eligiendo \texttt{target\_id} o \texttt{source\_id} según $\ell$ sea \textit{forward} o
\textit{backward}. Las secciones~\ref{sec:filtrado} y~\ref{sec:navegacion_transversal} del capítulo de desarrollo
describen esa traducción a SQL con el código a la vista, y la sección~\ref{sec:operaciones_conjuntos} hace lo
propio para la unión.

\subsubsection*{Semántica de la unión}

El operador de unión combina el estado activo con todos los estados guardados en $U$. Su semántica es la unión
conjuntista de los resultados individuales:
\[
  \llbracket \Sigma \rrbracket_{\cup} \;=\; \llbracket s_{\text{cur}} \rrbracket \;\cup\; \bigcup_{s \in U} \llbracket s \rrbracket.
\]

En el código, esta semántica se realiza en \texttt{NavigationDAO.getUnionEntities}, que emite una única consulta
SQL en la que cada estado de $U \cup \{s_{\text{cur}}\}$ contribuye con un bloque de condiciones unidas mediante
\texttt{OR}, exactamente la traducción de la unión definida arriba (sección~\ref{sec:operaciones_conjuntos}).
