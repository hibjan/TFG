...
en \texttt{StateManager}, reproducida en el Listado~\ref{lst:link}, tiene tres efectos.
...
almacenada en ese \texttt{Link}, según se reproduce en el Listado~\ref{lst:goback}.
...
La operación \texttt{union} en \texttt{StateManager}, cuyo cuerpo se reproduce en el
Listado~\ref{lst:union}, tiene cuatro efectos sobre el estado interno.
...
añadiendo un nuevo \texttt{Link} con la dirección invertida en lugar de eliminar el
último, lo que preserva el historial completo de forma creciente, según se muestra en
el Listado~\ref{lst:gobackSimetrico}.

\begin{lstlisting}[style=javaStyle, caption=Versión final simétrica de \texttt{goback}, label=lst:gobackSimetrico]
public void goback() {
    Link last = this.linkList.get(this.linkList.size() - 1);
    if (last.isForward()) {
