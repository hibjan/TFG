...
científicas, autores e instituciones. La elección deliberada de dos dominios separados ha
permitido comprobar empíricamente que las primitivas del motor (filtrado facetado, navegación
...
detallan en el capítulo~\ref{cap:desarrolloImplementacion} y no se reproducen aquí. El resto
del presente capítulo se concentra en la evaluación con usuarios, donde reside el grueso de
la evidencia empírica recogida.
...
\hline
\end{tabularx}
\caption{Perfil demográfico y de experiencia previa de los cinco participantes. La columna
...
\end{axis}
\end{tikzpicture}
\caption{Distribución de la completitud por tarea. Las tareas T4 (filtros de exclusión) y
T12 (reordenación de paneles) concentran la totalidad de las situaciones que no resultaron
en éxito limpio.}
...
\hline
\end{tabular}
\caption{Tiempos por tarea (segundos) y número de éxitos limpios sobre cinco intentos. Los
...
\addplot[red,dashed,thick,mark=none] coordinates {(0.5,5.5) (14.5,5.5)};
\addplot[gray,dashed,thick,mark=none] coordinates {(0.5,4.0) (14.5,4.0)};
\node[red,font=\scriptsize] at (axis cs:13.7,5.8) {Sauro 5{,}5};
\node[gray,font=\scriptsize] at (axis cs:13.7,4.3) {Aceptable 4};
\end{axis}
\end{tikzpicture}
\caption{Facilidad percibida (SEQ) media por tarea. Las líneas discontinuas marcan el umbral
de aceptación clásico (SEQ~$\geq$~4) y la referencia de Sauro para tareas \og fáciles\fg{}
...
clásico de 68 que separa los sistemas \og aceptables\fg{} de los que no lo son, y cuatro
de las cinco superan el umbral de 80 que se asocia con sistemas \og excelentes\fg{}. La
...
\addplot[red,dashed,thick,mark=none] coordinates {(P01,68) (P05,68)};
\addplot[green!50!black,dashed,thick,mark=none] coordinates {(P01,80) (P05,80)};
\node[red,font=\scriptsize] at (axis cs:P01,71) {Aceptable 68};
\node[green!50!black,font=\scriptsize] at (axis cs:P01,83) {Excelente 80};
\end{axis}
\end{tikzpicture}
\caption{Puntuación SUS por participante. La media del grupo es 85, con todos los valores
...
\hline
\end{tabularx}
\caption{Catálogo de incidencias detectadas, ordenadas por gravedad. La gravedad sigue
...
directamente sobre la etiqueta verde con \og+\fg{}, esperando que mutase a la versión roja
con \og$-$\fg{}. El control real (un botón segmentado en la cabecera del panel de Filtros)
...
mencionan literalmente este punto, y P01 lo expresa con claridad: \og piensa que sería más
intuitivo poder dar click al "+" de tal manera que se convierta en un "-"\fg{}. Adicionalmente,
...
tarea con los controles visibles. P02 sintetizó el problema durante la sesión: \og siente
que la interfaz es un poco grande y hay que ir moviéndose por ella\fg{}.
...
sesión, dos no consiguieron articular su semántica más allá de \og se han guardado cosas\fg{}.
...
filtro activo se comporte como un toggle: pulsar sobre el \og+\fg{} verde lo convertiría en
\og$-$\fg{} rojo y viceversa, de manera que el control segmentado actual quede como un valor
...
ya sea mediante etiquetas más explícitas (por ejemplo, \og Deshacer último link\fg{} y
\og Restaurar estado guardado\fg{}), un \textit{tooltip} la primera vez que el usuario pasa
...
entidades quede dentro del primer pliegue; introducir un indicador visual de \og hay más
contenido más abajo\fg{}; o reorganizar el orden por defecto de manera que las áreas de
...
introducción contextual la primera vez que el usuario pulsa \og + Add current\fg{} (un
...
\hline
\end{tabularx}
\caption{Resumen de casos de uso validados. La columna \textit{Veredicto} distingue entre
