...
Hemos realizado la validación funcional del motor de manera incremental durante el desarrollo,
...
nos ha permitido comprobar empíricamente que las primitivas del motor (filtrado facetado, navegación
...
detallan en el capítulo~\ref{cap:desarrolloImplementacion}; en lo que sigue aportamos evidencia
...
los dos \textit{datasets} disponibles. La lista se construye dinámicamente a partir de la tabla
\texttt{datasets} de PostgreSQL, de manera que añadir un nuevo dominio solo requiere extender
los datos, no la interfaz. Una vez elegido uno, la pantalla siguiente expone las colecciones
publicadas por ese \textit{dataset} (figura~\ref{fig:seleccionColecciones}): la disposición, tipografía
y controles son idénticos en ambos casos; lo único que cambia es el conjunto de colecciones
ofrecidas, que proviene de la tabla \texttt{collections} poblada por los \textit{pipelines} de ingesta
...
componente sirva para cualquier \textit{dataset} sin código específico.
...
Diseñamos la evaluación con cinco objetivos explícitos que guían tanto el diseño del estudio
...
34~minutos, en línea con la estimación previa de 35--45~minutos. Realizamos las sesiones
...
fueron grabadas con permiso explícito del participante, y eliminamos las grabaciones
una vez codificadas las observaciones.

\subsection{Perfil de los participantes}

Reclutamos cinco participantes con perfiles diversificados, siguiendo la recomendación de
...
\end{axis}
\end{tikzpicture}
\caption[Distribución de la completitud por tarea]{Distribución de la completitud por tarea. Las tareas T4 (filtros de exclusión) y
...
\hline
\textit{Toggle} \textit{Include}/\textit{Exclude} poco descubrible          & 3 (mayor)   & 5/5 & T4 \\
...
El \textbf{\textit{toggle} de inclusión y exclusión} es el problema más nítido del estudio. Los cinco
...
combinación de un \textit{toggle} separado del filtro y una previsualización ambigua acumula dos
...
filtro activo se comporte como un \textit{toggle}: pulsar sobre el «+» verde lo convertiría en
...
ya sea mediante etiquetas más explícitas (por ejemplo, «Deshacer último \textit{link}» y
