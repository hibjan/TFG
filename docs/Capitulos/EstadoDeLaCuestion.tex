...
décadas de investigación que indaga cómo permitir que un usuario explore colecciones digitales de tamaño
...
objeto de la colección se describe mediante un conjunto de etiquetas, 
y la interacción del usuario consiste en seleccionar una o varias de esas etiquetas para
...
estados son conjuntos de objetos y cuyas transiciones son las etiquetas en dicha propuesta.
...
El primero y más antiguo de estos modelos es el HAM (\textit{Hypertext Abstract Machine})
\citep{campbell1988ham}, concebido
...
entidad un conjunto de pares atributo-valor sobre los que opera el filtrado clásico. La
Tabla~\ref{tab:hamMapping} sintetiza esta correspondencia.

\begin{table}[h]
  \centering
  \begin{tabular}{@{}p{0.22\textwidth}p{0.34\textwidth}p{0.38\textwidth}@{}}
    \toprule
    \textbf{Objeto HAM} & \textbf{En nuestro sistema} & \textbf{Nota} \\
    \midrule
    Grafo               & tablas \texttt{collections} y \texttt{reference}
                        & Universo global de entidades y vínculos tipados \\
    Nodo                & filas de \texttt{entities}
                        & Unidad atómica del dominio \\
    Enlace              & filas de \texttt{reference}
                        & Asociaciones bidireccionales con razón \\
    Atributo semántico  & filas de \texttt{metadata}
                        & Pares atributo-valor para filtrado \\
    Contexto            & clase \texttt{State}
                        & Vista por sesión, separada del grafo persistente \\
    \bottomrule
  \end{tabular}
  \caption[Correspondencia HAM]{%
    Correspondencia entre los cinco objetos básicos del modelo HAM
    \citep{campbell1988ham} y los componentes equivalentes de nuestro
    sistema.}
  \label{tab:hamMapping}
\end{table}
...
(\textit{Dexter Hypertext Reference Model}) \citep{halasz1994dexter}. Dexter organiza el sistema de hipermedia en tres capas
...
\textit{anclajes} (\textit{anchors}) (Figura~\ref{fig:dexterCapas}). Esta triple separación encuentra paralelismos en la arquitectura de
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{Imagenes/Bitmap/dexter_capas.png}
  \caption[Capas del modelo Dexter]{Las tres capas del modelo Dexter,
    con las interfaces \textit{Presentation Specifications} y
    \textit{Anchoring} entre ellas. Figura tomada de Halasz y Schwartz
    (1994) \citep{halasz1994dexter}.}
  \label{fig:dexterCapas}
\end{figure}
...
de repositorios de objetos de aprendizaje basadas en esquemas fijos como LOM \citep{ieee_lom_2002}
o IMS CP \citep{ims_cp_2004}, es
...
por los usuarios. Esta filosofía \textit{bottom-up} acerca a Clavy a las folksonomías y, más en general, a los
...
finito independiente de la jerarquía (Figura~\ref{fig:clavyAutomata}). Dado que ese autómata puede, en el peor caso, crecer
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\textwidth]{Imagenes/Bitmap/clavy_automata.png}
  \caption[Autómata de navegación de Clavy]{Autómata de navegación de
    Clavy: los estados son conjuntos de objetos y las transiciones
    están etiquetadas con pares atributo-valor. Figura tomada de
    Gayoso et al. (2016) \citep{gayoso2016siie}.}
  \label{fig:clavyAutomata}
\end{figure}
...
acercándolo al espíritu de los lenguajes generalizados de marcado \citep{coombs1987markup}. La aplicabilidad práctica de la
...
estructura subyacente. Nuestro motor, en cambio, asume desde el principio un \textit{dataset} estratificado
en varias colecciones de naturaleza distinta (por ejemplo, películas, series, personas, productoras,
...
colección a otra siguiendo una relación, preservando los filtros del tramo previo en el \textit{snapshot} del enlace
para que la cadena completa de tramos se materialice en subconsultas SQL anidadas. A estas dos primitivas se añade la
...
(Tabla~\ref{tab:clavyComparativa}).

\begin{table}[h]
  \centering
  \begin{tabular}{@{}p{0.20\textwidth}p{0.36\textwidth}p{0.36\textwidth}@{}}
    \toprule
    \textbf{Eje} & \textbf{Clavy} & \textbf{Nuestro motor} \\
    \midrule
    Estructura de la colección
      & Homogénea, esquema único reconfigurable
      & Estratificada en subcolecciones tipadas con relaciones explícitas \\
    Primitivas de navegación
      & Filtrado por etiquetas
      & Filtrado por etiquetas y referencias, navegación transversal, y operación de unión \\
    Esquema
      & \textit{Bottom-up}, reconfigurable en caliente
      & Estable, definido a priori \\
    Estrategia de indexación
      & Autómatas, dendrogramas, cachés selectivas
      & Delegada en PostgreSQL; foco en traducción a SQL \\
    \bottomrule
  \end{tabular}
  \caption[Comparativa Clavy]{Síntesis de las
    diferencias entre la plataforma Clavy y nuestro motor de
    exploración a lo largo de cuatro ejes clave.}
  \label{tab:clavyComparativa}
\end{table}
...
mediante \textit{clustering} probabilístico (modelos de mezcla gaussiana sobre \textit{embeddings} reducidos con UMAP),
...
(Figura~\ref{fig:knowledgeNavFlujo}).

\begin{figure}[h]
  \centering
  \includegraphics[width=0.95\textwidth]{Imagenes/Bitmap/knowledgenav_workflow.png}
  \caption[Flujo de Knowledge Navigator]{Flujo de Knowledge Navigator:
    recuperación, \textit{embedding} y \textit{clustering}, lectura
    por el modelo del lenguaje y organización temática final. Figura
    tomada de Katz et al. (2024) \citep{katz2024knowledgenav}.}
  \label{fig:knowledgeNavFlujo}
\end{figure}
...
de él para reducir el problema a uno de \textit{clustering} sobre \textit{embeddings} supondría desperdiciar
